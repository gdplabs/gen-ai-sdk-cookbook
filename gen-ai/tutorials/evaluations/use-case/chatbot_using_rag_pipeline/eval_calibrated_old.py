"""Calibrated evaluation script using a mock pipeline with fixed outputs.

Identical to eval.py but with completeness threshold lowered from 1.0 to 0.5.
Calibration rationale: for browse queries, the client confirmed that partial
coverage (>=50%) is sufficient. See tutorial-rag-agent-evals.md Step 5.
"""
...
async def main():
    ...
    # completeness threshold lowered from 1.0 → 0.5 after client review:
    # browse queries require representative coverage only, not exhaustive.
