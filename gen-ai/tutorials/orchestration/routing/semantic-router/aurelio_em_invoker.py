"""Semantic Router — Aurelio backend with an EM Invoker.

Uses ``SemanticRouter.aurelio()`` with a ``BaseEMInvoker`` passed directly. In
v0.5+ the EM invoker is auto-wrapped in an ``EMInvokerEncoder``.

In production the encoder is a real embedding model:
...
``FakeEMInvoker`` (see ``fake_em.py``). The ``SemanticRouter``, the Aurelio
adapter, and the Aurelio Labs ``semantic-router`` library are the real code path.

Based on the "Aurelio Backend with EM Invoker" section of the GitBook page:
