"""Evaluator for query ID 6: Price per share paid to FirmGlobal1OY in each investment round."""

from utils import check_currency_value_in_string, iter_messages
from .base import BaseQueryEvaluator, log_metric

_TARGET_COMPANY = "FirmGlobal1OY"
_EXPECTED_PPS = "85,778"


class QueryEvaluator006(BaseQueryEvaluator):
    """Evaluator for query ID 6.

    Query: What was the price per share we paid for FirmGlobal1OY in each of its
    investment rounds?

    Expected: Series D round on August 30, 2016 — price per share of $85,778.00.

    Metrics applied per HIGH_ALIGNMENT_METRIC_GUIDE.MD:

    Base class (inherited):
        - metric_has_answer       — non-empty answer (#2 Task Success proxy)
        - metric_keyword_match    — required keywords present (date + figure + entity + round)
    ...
    Custom (this class):
        ...
        - metric_sql_targeted_correct_entity    — #13 Tool Calling Correctness / #15 SQL Correctness:
                                                SQL query targeted 'FirmGlobal1OY'
    ...
    def metric_sql_targeted_correct_entity(self, record: dict) -> bool:
        ...
        a SQL query targeting 'FirmGlobal1OY'.
    ...
    def metric_answer_grounded_in_tool_output(self, record: dict) -> bool:
        ...
        Verifies that the expected PPS value (85778) appears in at least one tool
