"""Evaluator for query ID 8: Current valuation for BizGroupYPZ."""
...
_EXPECTED_VALUATION = "70,782,486,011"


class QueryEvaluator008(BaseQueryEvaluator):
    """Evaluator for query ID 8.

    Query: What are the current valuations for BizGroupYPZ?

    Expected: Current valuation of approximately $70.8 billion ($70,782,486,011 USD).

    Metrics applied per HIGH_ALIGNMENT_METRIC_GUIDE.MD:

    Base class (inherited):
        - metric_has_answer       — non-empty answer (#2 Task Success proxy)
        - metric_keyword_match    — required keywords present (entity + figure)
    ...
    Custom (this class):
        ...
        - metric_sql_targeted_correct_entity    — #13 Tool Calling Correctness / #15 SQL Correctness:
                                                SQL query targeted 'BizGroupYPZ'
    ...
    def metric_sql_targeted_correct_entity(self, record: dict) -> bool:
        ...
        a SQL query targeting 'BizGroupYPZ'.
    ...
    def metric_answer_grounded_in_tool_output(self, record: dict) -> bool:
        ...
        Verifies that the expected numeric value (70782486011) appears in at least one
        tool result message within the trajectory, confirming the answer is grounded in
        ...
        for msg in iter_messages(messages):
            if msg.get("role") == "tool":
                ...
                if check_currency_value_in_string(_EXPECTED_VALUATION, content):
                    return True
        return False
